...
- **Responses that mix `start_task(x)` with other tools split at the pivot.** The Context captures the owning task id immediately before each dispatch, then records the response in groups: `start_task` lands outside the new task (so task boundaries stay visible in history), while anything that follows lands inside it (so that work collapses when the task is closed).

The UI lays the ConversationLog out as a conversation stream followed by a pending-tasks panel at the bottom (only `PENDING` tasks — the "Add a task" input sits there). `IN_PROGRESS` tasks render inline in the stream as a banner with their memory block and a close button. `DONE` tasks render inline as collapsible `<details>` blocks with an independent `ui_expanded` toggle that has no effect on the LLM's view. A 🔍 marker indicates a task the LLM has picked for expansion on the next request. Consecutive tool-call / tool-return runs between agent turns are wrapped in a `<details>` closed by default, summarised by the number of tool calls.
